...
cv::Mat YoloX::call(cv::Mat& img, Ort::MemoryInfo& mem_info, Ort::RunOptions& run_options,
...
std::vector<Box> YoloX::call_box(cv::Mat& img, Ort::MemoryInfo& mem_info, Ort::RunOptions& run_options,
    transforms::DataFormat data_format) {
    auto result = forward(img, mem_info, run_options, data_format);
    return postprocess(result);
}

void YoloX::adjust_par(int h, int w, float nms_thresh, float confidence_thresh) {
    this->h = h;
    this->w = w;
    this->nms_thresh = nms_thresh;
    this->confidence_thresh = confidence_thresh;
...
cv::Mat YoloX::preprocess(cv::Mat& mat, transforms::DataFormat data_format) {
    ...
    cv::Mat resize_img = transforms::resize(mat, false, w, h, &ratio,false,cv::Scalar::all(114.));
    return resize_img;
}

std::vector<Ort::Value> YoloX::forward(cv::Mat& img, Ort::MemoryInfo& mem_info, Ort::RunOptions& run_info,
    transforms::DataFormat data_format) {
...
std::vector<Box> YoloX::postprocess(std::vector<Ort::Value>& result) {
