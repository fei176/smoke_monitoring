...
namespace transforms {
	...
	cv::Mat normalize(const cv::Mat& mat, const std::vector<float>& mean, const std::vector<float>& std = {1.,1.,1.});
	...
	cv::Size get_new_size(cv::Mat& img, int max_size, float* ratio_value,bool only_zoom_out=true);
